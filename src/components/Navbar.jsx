...
const Navbar = () => {
  ...
  useEffect(() => {
    setAuthToken(
      isAuthenticated ? localStorage.getItem("user_token") || "1" : null
    );
  }, [isAuthenticated]);

  // Handle scroll event to toggle scrolled style
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <nav
      className={`${
        isMenuOpen
          ? "fixed top-0 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 shadow-sm "
          : isHome
          ? isScrolled
            ? "fixed top-0 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 shadow-sm "
            ...
            <form onSubmit={handleSearch} className="w-full">
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-2 pl-10 pr-4 text-black rounded-full shadow
            ...
            {categories.map((cat, idx) => {
              ...
              return (
                <Link
                  ...
                  to={`/${routeName}`}
                  className="text-gray-900 hover:text-emerald-800 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 capitalize"
                >
                  {name}
                </Link>
              );
            ...
            {isAuthenticated ? (
              <div className="relative" ref={profileMenuRef}>
                <button
                  ...
                  className="flex items-center text-gray-700 hover:text-emerald-800 px-3 py-2 rounded-md transition-colors duration-200"
                >
                  <FaUserCircle size={26} />
                </button>
                {/* Dropdown */}
                <div
                  className={`absolute z-[60] right-0 mt-2 w-44 origin-top-right rounded-xl border border-gray-100 bg-white shadow-lg transition-all duration-200 ${
                    isProfileOpen
              ...
              {categories.map((cat, idx) => {
                ...
                return (
                  <Link
                    ...
                    className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-emerald-600 hover:bg-gray-100 transition-colors duration-200 capitalize"
                  >
                    {name}
                  </Link>
                );
              ...
              {isAuthenticated ? (
                <div className="px-2 pt-2">
                  <Link
                    onClick={() => setIsMenuOpen(false)}
                    to="/profile"
                    className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-emerald-600 hover:bg-gray-100 transition-colors duration-200"
                  >
                    Profile
                  </Link>
                  <button
                    ...
                    className="mt-1 block w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-emerald-600 hover:bg-gray-100 transition-colors duration-200"
                  >
                    Logout
                  </button>
                </div>
                ...
                <div className="flex items-center gap-3 px-2 pt-2">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-md text-base font-medium text-emerald-700 border border-emerald-600 hover:bg-emerald-50 transition-colors duration-200"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-md text-base font-medium text-white bg-emerald-600 hover:bg-emerald-700 transition-colors duration-200"
                  >
                    <FaUserCircle size={18} />
                    Sign Up
                  </button>
