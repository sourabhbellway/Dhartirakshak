...
const RightSidebar = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(e, "Failed to load advertisements");
      } finally {
        setLoading(false);
      }
    };
  ...
  const advertisements = (filteredAds.length ? filteredAds : []).filter(
    (ad) => !dismissedIds.has(String(ad.id || ad._id))
  );
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50  border-l lg:border-l border-t lg:border-t-0 border-gray-200 p-3 sm:p-4">
      {/* Header */}
          ...
          <div className="space-y-4">
            {advertisements.map((ad) => {
              ...
              return (
                <div
                  key={ad.id || ad._id}
                  className="relative bg-white rounded-lg p-3 sm:p-3 border border-gray-200 hover:shadow-md transition-all duration-200"
                >
                  {/* Dismiss (X) */}
                  <button
                    ...
                    onClick={() =>
                      setDismissedIds((prev) =>
                        new Set(prev).add(String(ad.id || ad._id))
                      )
                    }
                  >
                    <FaTimes />
                  ...
                  {/* Ad badge */}
                  <div className="absolute -top-2 left-3 bg-yellow-500 text-white text-[9px] px-2 py-0.5 rounded-full shadow animate-pulse">
                    Ad
                  </div>
                      ...
                      (targetUrl ? (
                        ...
                        >
                          <img
                            src={ad.image}
                            alt={ad.title || "ad"}
                            className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-md border border-green-200"
                          />
                        </a>
                      ) : (
                        <img
                          src={ad.image}
                          alt={ad.title || "ad"}
                          className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-md border border-green-200 shrink-0"
                        />
                      ))}
                      ...
                      {ad.title ? (
                        targetUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="font-semibold text-gray-900 text-sm mb-1 line-clamp-2 block hover:underline"
                          >
                            {ad.title}
                          </a>
                        ) : (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {targetUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="ml-2 text-[11px] text-dark-green hover:underline whitespace-nowrap"
                          >
                            Visit site
                          </a>
                        )}
      ...
      <a
        ...
        className="block"
      >
        <div className="rounded-2xl   flex items-center justify-center rounded-2xl overflow-hidden h-30">
          <img
            src={sidebanner}
            alt="Bellway Infotech"
